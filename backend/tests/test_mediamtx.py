...
# ─── self-heal: ensure_stream (mediamtx 재시작 후 누락 path 자가복구) ───


def _db_with_cam(stream_key: str, rtsp_url: str):
    """get(IpCam).filter_by(stream_key=...).first() 가 카메라를 반환하는 mock Session."""
    cam = MagicMock()
...
def test_ensure_stream_reregisters_when_path_missing():
    """mediamtx 에 path 가 없으면(get_path None) DB 의 rtsp_url 로 재등록한다."""
    db, _ = _db_with_cam("ipcam-abc", "rtsp://cam/stream1")
    with patch("app.mediamtx.get_path", return_value=None), \
         patch("app.mediamtx.register_stream", return_value=True) as reg:
        result = mediamtx.ensure_stream(db, "ipcam-abc")
        assert result is True
        reg.assert_called_once_with("ipcam-abc", "rtsp://cam/stream1")


def test_ensure_stream_noop_when_path_present():
    """path 가 이미 있으면 재등록하지 않는다(불필요 POST 회피)."""
    db, _ = _db_with_cam("ipcam-abc", "rtsp://cam/stream1")
    with patch("app.mediamtx.get_path", return_value={"name": "ipcam-abc"}), \
...
def test_ensure_stream_noop_when_camera_unknown():
    """DB 에 없는 stream_key 면 재등록 시도 안 함(404 카메라 등)."""
    db = MagicMock()
    db.query.return_value.filter.return_value.first.return_value = None
    with patch("app.mediamtx.get_path", return_value=None), \
         patch("app.mediamtx.register_stream") as reg:
        result = mediamtx.ensure_stream(db, "ipcam-gone")
        assert result is False
        reg.assert_not_called()
...
def test_register_masks_password_in_log(caplog):
    ...
    with patch.object(mediamtx, "MEDIAMTX_API", "http://mtx:9997"), \
         patch("app.mediamtx.httpx.post") as post:
        post.return_value = MagicMock(status_code=200)
        with caplog.at_level(logging.INFO, logger="cam-distance.mediamtx"):
