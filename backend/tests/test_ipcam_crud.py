...
def test_register_over_cap_409_releases_write_lock(client):
    ...
    ids = []
    for i in range(MAX_IPCAMS):
        r = client.post("/api/ipcams", json={"name": f"cam{i}", "rtsp_url": f"rtsp://x/{i}"})
        assert r.status_code == 201
        ids.append(r.json()["id"])

    # cap 초과 → 409 (BEGIN IMMEDIATE 선점 후 rollback)
    over = client.post("/api/ipcams", json={"name": "over", "rtsp_url": "rtsp://x/over"})
    assert over.status_code == 409

    # 락이 해제됐어야 후속 쓰기 성공: 하나 지우고 다시 등록 → 201 (wedge 아님)
    assert client.delete(f"/api/ipcams/{ids[0]}").status_code == 204
    again = client.post("/api/ipcams", json={"name": "reslot", "rtsp_url": "rtsp://x/reslot"})
    assert again.status_code == 201
    assert len(client.get("/api/ipcams").json()) == MAX_IPCAMS


# ─── net-new: update register-fallback — PATCH 실패 시 register 재생성 폴백 (lineage fix) ───
