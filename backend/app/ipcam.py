...
from app.mediamtx import get_path, register_stream, remove_stream, ensure_stream
...
def update_ipcam(cam_id: int, body: IpCamUpdate, db: Session = Depends(get_db)) -> IpCam:
    ...
    되돌리고 host/path 등 사용자가 바꾼 주소는 그대로 반영한다. 비밀번호를 바꾸려면 *** 를
    지우고 새 비밀번호를 입력한다.
    """
    ...
    # *** 마스킹된 비밀번호만 기존 실제값으로 복원, host/path 등 수정은 보존(주소 변경 버그 수정).
...
def get_ipcam_stats(stream_key: str, db: Session = Depends(get_db)) -> dict:
    ...
    self-heal: mediamtx 가 독립 재시작돼 path 가 사라졌으면 여기서 자가복구
    재등록한다. 프론트(CamerasPage)가 이 엔드포인트를 1초 폴링하므로, mediamtx
    재시작 후 backend 재시작 없이 ~1초 내 path 가 복원된다(grid·measure 양쪽 커버).
    """
    ensure_stream(db, stream_key)
...
def update_ipcam_calibration(
    ...
    result = _compute_homography(body.pixel_points, body.world_points)
    poly = _compute_polynomial(body.pixel_points, body.world_points)
    ...
    logger.info(
        "IP CAM calibration 갱신: stream_key=%s enabled=%s mean_reproj=%.2fpx poly_err=%.4fm",
        stream_key, body.enabled, result.mean_reprojection_error, poly["poly_mean_error_m"],
    )
    return state
