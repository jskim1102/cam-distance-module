...
logger = logging.getLogger("cam-distance.mediamtx")
...
def _validate_rtsp_url(rtsp_url: str) -> None:
    ...
    if any(c in rtsp_url for c in _FORBIDDEN):
        raise ValueError(f"rtsp_url 에 금지문자(공백/제어/셸메타)가 포함돼 있습니다: {mask_rtsp_credentials(rtsp_url)!r}")
...
def ensure_stream(db: Session, stream_key: str) -> bool:
    """self-heal — mediamtx 에 path 가 없으면 DB 의 rtsp_url 로 재등록.
    ...
    반환: 재등록을 수행했으면 True, path 가 이미 있거나 카메라 미존재면 False.
    """
    if get_path(stream_key):
        return False  # 이미 등록됨 — 불필요한 POST 회피
    cam = db.query(IpCam).filter(IpCam.stream_key == stream_key).first()
    if not cam:
        return False  # DB 에 없는 카메라 — 재등록 대상 아님
    logger.info("mediamtx path 누락 감지 → 자가복구 재등록: %s", stream_key)
    return register_stream(cam.stream_key, cam.rtsp_url)
