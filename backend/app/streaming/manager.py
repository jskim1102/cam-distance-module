...
class StreamManager:
    ...
    def _record_results(self, results: list[InferenceResult]) -> None:
        ...
        worker 전역 service-time은 삭제 직전 결과도 유효하지만, source별 상태와 bbox는
...
def detections_to_json(result: InferenceResult) -> str:
    ...
    좌표 xyxy 는 추론 캡처 frame 픽셀 기준. `frame:{w,h}` 동봉(SEAM) — frontend
    BboxOverlay 가 video.videoWidth/frame.w 스케일로 변환(두 해상도 같으면 identity).
    """
    return json.dumps({
        "type": "detections",
