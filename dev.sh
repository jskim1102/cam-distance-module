...
project_services() {
  ...
  SERVICES=(
    "backend|backend|source .venv/bin/activate && export MEDIAMTX_API='http://127.0.0.1:${MEDIAMTX_API_PORT}' && export MEDIAMTX_PATH_PREFIX=cam_distance && alembic upgrade head && exec uvicorn app.main:app --host 0.0.0.0 --port ${BACKEND_PORT} --reload"
    "frontend|frontend|VITE_API_PORT='${BACKEND_PORT}' VITE_MEDIAMTX_WEBRTC_PORT='${MEDIAMTX_WEBRTC_PORT}' VITE_MEDIAMTX_VIEWER_USER='${MEDIAMTX_VIEWER_USER:-viewer_cam_distance}' VITE_MEDIAMTX_VIEWER_PASS='${MEDIAMTX_VIEWER_PASS:-}' exec npm run dev -- --host 0.0.0.0 --port ${FRONTEND_PORT}"
  )
  PORTS=("${BACKEND_PORT}" "${FRONTEND_PORT}")
}

pre_up()    {
  # requirements 변경분을 서버 시작 전 동기화 (해시 같으면 skip — 재시작 빠르게).
  local req=backend/requirements.txt stamp=backend/.venv/.requirements.sha256
  [ -x backend/.venv/bin/python ] || { echo "backend/.venv 없음 — venv 먼저 생성" >&2; exit 1; }
  local h; h=$(sha256sum "$req" | awk '{print $1}')
  local cur=""; [ -f "$stamp" ] && cur=$(tr -d '\r\n' < "$stamp")
  if [ "$h" != "$cur" ]; then
    echo "backend requirements 변경 감지 — .venv 동기화"
    backend/.venv/bin/python -m pip install -r "$req"
    # ultralytics 가 GUI용 opencv-python 을 끌어옴 — 서버 정본은 headless (Dockerfile 동치).
